...
final class BlackListVC: BaseViewController {

    private let image = UIImageView().then {
        $0.image = .image.banned.image
    }
    ...
    private let mainLabel = UILabel().then {
        ...
        $0.text = "외출 금지 상태에서는 외출이 불가능합니다.\n다음 외출제를 이용해 주세요."
        $0.setLineSpacing(spacing: 3)
        $0.numberOfLines = 2
        $0.textAlignment = .center
    }
    
    private lazy var checkButton = GOMSButton(frame: CGRect(x: 0, y: 0, width: 0, height: 0), title: "홈으로 돌아가기").then {
